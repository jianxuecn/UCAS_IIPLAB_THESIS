...
带编号的数学公式一般使用\ltxenvname{equation}环境，需要在正文中引用的在环境内用\ltxcmdname{label}命令设置引用标签：
\begin{equation}\label{eq:samples:one}
F_n=\sum_{i=0}^{\infty} \binom{n-i}{i}
\end{equation}

带编号的数学公式组使用\ltxenvname{align}环境（注意利用列分隔符“\texttt{\&}”对齐）：
\begin{align}
\mathcal{M}^2(\hat{\theta},\theta) &= E[(\hat{\theta} - \theta)^2] \\
\mathcal{M}^2(\hat{\theta},\theta) &= \operatorname{var}^2(\hat{\theta}) + {\cal B}^2(\hat{\theta})
\end{align}

还可用\ltxenvname{equation}内套一个\ltxenvname{aligned}环境合并公式组，统一给一个编号：
\begin{equation}
\begin{aligned}
\mathcal{M}^2(\hat{\theta},\theta) &= E[(\hat{\theta} - \theta)^2] \\
\mathcal{M}^2(\hat{\theta},\theta) &= \operatorname{var}^2(\hat{\theta}) + {\cal B}^2(\hat{\theta})
...
跨行长公式在\ltxenvname{equation}环境内套一个\ltxenvname{split}环境（注意利用“\texttt{\&}”对齐）：
\begin{equation}
\begin{split}
a& =b+c-d\\
 & \quad +e-f\\
...
带子编号的数学公式组在\ltxenvname{align}环境\textbf{外}套一个\ltxenvname{subequations}环境：
...
带条件赋值在\ltxenvname{equation}环境内使用\ltxenvname{array}环境：
\begin{equation}
z = \left\{
\begin{array}{ll}
 1 & (x>0)\\
...
也可以在\ltxenvname{equation}环境内用\ltxenvname{cases}环境：
\begin{equation}
z = 
\begin{cases}
 1 & (x>0)\\
...
矩阵的输入用 \ltxenvname{matrix} 环境，以“\texttt{\&}”分列、以“\ltxcmdname{\textbackslash}”或“\ltxcmdname{cr}”分行，下面是一个例子：
\begin{equation}
\left(
  \begin{matrix}
...
也可直接用带括号的版本，如\ltxenvname{pmatrix}（小括号）、\ltxenvname{bmatrix}（中括号）、\ltxenvname{Bmatrix}（大括号）、\ltxenvname{vmatrix}（单竖线）、\ltxenvname{Vmatrix}（双竖线）等：
\begin{equation}
  \begin{bmatrix}
    K_{11} &K_{12} &K_{13} &\ldots &K_{1n}\\
    K_{21} &K_{22} &K_{23} &\ldots &K_{2n}\\
...
\begin{enumerate}
  \item \textbf{多字母的函数或算子名称表示要规范}：多字母的函数或算子名称在~\LaTeX{}中有专门的排版方式，一些常用的已经定义了专门的命令，
        例如\ltxcmdname{sin}、\ltxcmdname{max}、\ltxcmdname{log}等，对于一些\ltxenvname{amsmath}包中未定义的函数，
        一定要在数学环境中用\ltxcmdname{operatorname}命令括起来，
        例如：\ltxcmdname{operatorname\{softmax\}(A)}就显示为$\operatorname{softmax}(A)$，
        这和在数学环境中用\ltxcmdname{text}（显示为$\text{softmax}(A)$）或
        直接写（显示为$softmax(A)$）都有差别，后者尤为错误。
  \item \textbf{尽量不要使用多字母变量}：在数学环境中直接输出多字母变量会产生歧义，数学环境中默认的排版规则认为每一个英文字母都是一个单独的变量，
        在字符间距的处理上有特殊的考虑，若直接输入多字母变量，则各个字母间会分得比较开，例如：$Ver_i$，读者容易看成$V$、$e$和$r_i$的乘积，
        因此数学公式中要避免使用多字母变量。
        在这里，一条重要的原则是：数学公式要经过一定的提炼和抽象，不要直接从程序代码翻译，即不要在数学公式的表达中带入写程序的习惯。\label{enum:mathtips:1}
  \item \textbf{不同类型的数学符号尽量用不同的字体表示}：不同类型的数学符号用不同类型的字体表示有利于增加公式的辨识度，使读者更易理解，
        例如矩阵和向量一般用粗体，$\mathbf{Ax}=\mathbf{b}$就比$Ax=b$含义更明确。
  \item \textbf{用有意义的单词做变量上下标时单词要放在文本环境中}：有时候为了表达更简洁明了，会在变量的上标或下标用单词表示某些特殊含义，
        例如$A_\text{max}$，此时，用来表达特殊含义的单词务必放在\ltxcmdname{text}命令中，理由同~\ref{enum:mathtips:1}。
  \item \textbf{公式前面不要加空行}：一般公式内容是紧接上文的，逻辑上不应出现中断，因此公式应紧接前文，中间不要有空行。
        而从排版的角度讲，出现空行意味着另起一段，这会在公式前面出现不正常的多余空白，也不美观。
        至于公式之后加不加空行，视情况而定。如果后面的文字是直接对公式中的符号进行解释，即整句话还没有结束，则不加空行；
        如果后面的文字已经是另一段的开始，前面的内容到公式已经结束，则可加空行。
\end{enumerate}

\subsection{插图}\label{sec:fig}
为了能得到较好的排版效果，又不必做调整插图位置的乏味工作，\LaTeX{} 提供了一个浮动对象机制来自动将插图放置到合适的位置。为了使这一机制发挥最大效力，在写作时需注意以下几点：
\begin{enumerate}
	\item \textbf{不要使用依赖于插图放置位置的文本}：使用如“\emph{这幅图……}”或“\emph{下图……}”等短语要求所指插图需在固定位置，
	      而像“{\kaishu 图1-1……}”这样的短语则允许插图出现在任意位置，而且配合\LaTeX{} 提供的强大的交叉引用功能，
	      完全可以自动生成插图编号并建立超链接。
  \item \textbf{不要让插图出现的位置离引用位置过远}：虽然采用编号引用之后，理论上插图可以出现在论文的任何位置，但是请不要人为地制造阅读障碍，
        尽量使插图出现在正文第一次引用位置的附近，即在撰写论文正文时，在紧挨着初次引用某插图的段落之后（或之前）的位置放置该插图。
	\item \textbf{不要过早地去手动调整插图的放置位置}：一些刚开始使用\LaTeX{} 的作者发现插图没有十分准确地出现在他们所想要的位置，往往会着急，
        并对其进行手工干预，这其实完全没有必要，插图的放置是\LaTeX{} 的工作，最好放轻松一些，等所有内容都撰写校对完成后再对插图位置作必要微调。
	      如果论文结构合理，正文内容完整充实，此时就会发现插图都已在该在的位置上，没必要调整了。
\end{enumerate}
...
插图一般被放置在\ltxenvname{figure}环境中，\figref{fig:samples:coordinateaxis}是一个单幅插图的例子。\ltxenvname{figure}环境有一个可选参数允许用户指示插图有可能被放置的位置（实际上所有浮动对象环境均有类似的参数设置，例如\secref{sec:tab}要讲的表格环境），这一选项参数可以是下列字母的任意组合：
\begin{description}
...
如果不指定，默认使用\texttt{[tbp]}。通常给出的参数越多，\LaTeX{} 的排版效果越好，例如\texttt{[htbp]}、\texttt{[tbp]}、\texttt{[htp]}、\texttt{[tp]}等组合的效果都不错。另外，在这些位置参数前面加上一个惊叹号（例如\texttt{[!ht]}）会使\LaTeX{} 忽略应用于文本页的审美条件，试图用最严格的标准来放置浮动图形。
...
另外需\textbf{注意}：高分辨率的\texttt{png}格式文件会严重影响文档编译速度，建议转成\texttt{pdf}格式。
...
表格和插图一样，建议使用浮动对象，即将表格放在浮动对象环境\texttt{table}中，例如\tabref{tab:samples:normal}。\secref{sec:fig} 中提到的关于插图的排版原则大多数同样适用于表格，但是也有一些例外，例如跨页的长表格\texttt{longtable}（\tabref{tab:samples:longtable}），由于不受页面长度限制，所以不必放在浮动对象中，放进去反而会出错。
...
\texttt{longtable}环境一般用于排版一页放不下的超长表格，与普通表格排版方式不一样，不能当做浮动对象，当它附近有较多浮动表格对象时有可能导致表格出现的顺序和生成的编号顺序不一致，因此如无必要，建议尽量少用跨页长表，对于过长的表格应当进行适当的拆分。
